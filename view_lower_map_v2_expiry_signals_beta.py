...
if event.selection:
    ...
    if indices:
        try:
            ...
            if idx < len(gdf_view):
                ...
                # 4. Show Debug Info (Temporary)
                st.sidebar.error(f"🛠️ DEBUG: Clicked Index {idx} -> {clicked_muni}")
                
                # 5. Force Sidebar Update
    ...
    # Show passive state if no click
    st.sidebar.caption("Map ready. Click a municipality to test.")
