...
PROMPT_TEMPLATE = """# TASK: Extract Livestock Guardian Dog (LGD) Bylaw Data — P2 Broken Links Batch

## Context
I am a policy analyst at the Ontario Federation of Agriculture (OFA). We maintain a database tracking how all 444 Ontario municipalities regulate Livestock Guardian Dogs (LGDs) in their animal control bylaws. For the municipalities in this batch, the bylaw URL we had on file is broken or outdated. I need you to **search the web for each municipality's current animal control or dog licensing bylaw**, find the correct document, read it, and extract the data fields below.
...
def generate_p2_prompts():
    audit = pd.read_csv(os.path.join(SIGNALS_DIR, 'lgd_audit_2026-04-29.csv'))
    p2 = audit[audit['Audit Result'] == 'LINK BROKEN']
    
    batch_size = 10
    num_batches = math.ceil(len(p2) / batch_size)
    
    print(f"Generating {num_batches} P2 prompts for {len(p2)} municipalities...")
